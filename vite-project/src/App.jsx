...
const TARGET_CHAIN_ID_DEC = 2810; // Morph Holesky (some wallets may report 2818 in older configs)
const ALT_CHAIN_ID_DEC = 2818;
...
function App() {
  ...
  const ensureCorrectNetwork = async (eth) => {
    ...
    const isOk = currentChainIdHex === TARGET_CHAIN_ID_HEX || currentChainIdHex === ALT_CHAIN_ID_HEX;
    if (!isOk) {
      try {
        await eth.request({
